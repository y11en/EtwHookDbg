...
{
	EVENT_TRACE_PROPERTIES  eventTrace;
	ULONG64					Unknown[3];
	UNICODE_STRING			ProviderName;
} CKCL_TRACE_PROPERTIES, *PCKCL_TRACE_PROPERTIES;

BOOLEAN IfhpResolveSymbols();

NTSTATUS IfhpModifyTraceSettings(
	_In_ CKCL_TRACE_OPERATION Operation);

ULONG64 IfhpInternalGetCpuClock();
...
static BOOLEAN IfhpInitialized = FALSE;
static BOOLEAN IfhpInitializedContirol = TRUE;
...
static BOOLEAN IfhpResolveSymbols2();
	...
	INFINITYHOOKCALLBACK InfinityHookCallback)
{
	...
	*/
	...
	// this structure.
	//
	PVOID* AddressOfEtwpGetCycleCount = NULL;
	RTL_OSVERSIONINFOW version = { 0 };
	RtlGetVersion(&version);
	if(version.dwMajorVersion >6) AddressOfEtwpGetCycleCount = (PVOID*)((uintptr_t)CkclWmiLoggerContext + OFFSET_WMI_LOGGER_CONTEXT_CPU_CYCLE_CLOCK);
	else AddressOfEtwpGetCycleCount = (PVOID*)((uintptr_t)CkclWmiLoggerContext + 0x18);
	//
	// Replace this function pointer with our own. Each time syscall
	// is logged by ETW, it will invoke our new timing function.
	//
	
	if (MmIsAddressValid(AddressOfEtwpGetCycleCount))
	{
		//DbgBreakPoint();
...
BOOLEAN IfhpResolveSymbols()
{
	...
	ULONG64 kernelBase = 0, kernelSize = 0;
	NTSTATUS status = GetNtModuleBaseAndSizeEtw(&kernelBase, &kernelSize);
	if (!NT_SUCCESS(status))
	{
		return NULL;
	}

	PVOID NtBaseAddress = (PVOID)kernelBase;
	//ULONG SizeOfNt = (ULONG)kernelSize;
...
static BOOLEAN IfhpResolveSymbols2()
{
	//暴力搜索
	//DbgBreakPoint();
	static ULONG64 fundAddr = 0;
	if (!fundAddr)
	{
		FindCodeEtw findCode[2] = { 0 };
		initFindCodeStructETW(&findCode[0], "48890d****48890d****48890d****48890d****488d0d****33d2488905****48893d****e8", 0, 0x3D);
		initFindCodeStructETW(&findCode[1], "418d5c**488d3d****488bc38d4b*f348ab", 0x38, 0x3D);
		ULONG64 moduleAddr = 0;
		ULONG64 moduleSize = 0;
		if (!NT_SUCCESS(GetNtModuleBaseAndSizeEtw(&moduleAddr, &moduleSize)))
		{
			KdPrint(("获取模块失败\n"));
			return FALSE;
		}

		fundAddr = findAddressByCodeETW(moduleAddr, moduleAddr + moduleSize, findCode, 2);
		KdPrint(("找到函数%p\n", fundAddr));
		if (fundAddr == 0)
		{
			return FALSE;
	...
	ULONG offset = *((ULONG*)(fundAddr + 3));
	LARGE_INTEGER addr = { 0 };
	addr.QuadPart = fundAddr + 7;
	addr.LowPart += offset;


	PVOID* EtwpDebuggerDataSilo = (PVOID*)(addr.QuadPart);
	...
	CkclWmiLoggerContext = EtwpDebuggerDataSilo[INDEX_CKCL_LOGGER];
	//
	KdPrint(("CkclWmiLoggerContext找到结构%p\n", CkclWmiLoggerContext));
	...
	_In_ CKCL_TRACE_OPERATION Operation)
{
	...
	UNICODE_STRING unStr1 = RTL_CONSTANT_STRING(L"Circular Kernel Context Logger");
	

	Property->eventTrace.Wnode.BufferSize = PAGE_SIZE;
	Property->eventTrace.Wnode.Flags = WNODE_FLAG_TRACED_GUID;
	Property->ProviderName.MaximumLength = 0x256;
	Property->ProviderName.Buffer = (PWCHAR)ExAllocatePool(NonPagedPool, 0x256);
	memset(Property->ProviderName.Buffer, 0, 0x256);
	Property->ProviderName.Length = 0;
	RtlCopyUnicodeString(&Property->ProviderName, &unStr1);
	//Property->ProviderName = RTL_CONSTANT_STRING(L"Circular Kernel Context Logger");
	Property->eventTrace.Wnode.Guid = CkclSessionGuid;
	Property->eventTrace.Wnode.ClientContext = 1;
	Property->eventTrace.BufferSize = sizeof(ULONG);
	Property->eventTrace.MinimumBuffers = Property->eventTrace.MaximumBuffers = 2;
	Property->eventTrace.LogFileMode = EVENT_TRACE_BUFFERING_MODE;
	...
	if(version.dwMajorVersion == 6) wpoff();
	switch (Operation)
	{
		...
		case CKCL_TRACE_SYSCALL:
		{
			...
			//
			Property->eventTrace.EnableFlags = EVENT_TRACE_FLAG_SYSTEMCALL;
	...
	if (version.dwMajorVersion == 6) wpon();
